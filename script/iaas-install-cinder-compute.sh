...
pvcreate -f /dev/$BLOCK_DISK                          强制创建物理卷
vgcreate cinder-volumes /dev/$BLOCK_DISK				创建卷组cinder-volume

# sed -i  "/$BLOCK_DISK/d"  /etc/lvm/lvm.conf              删除 /etc/lvm/lvm.conf 文件中有$BLOCK_DISK 的行
